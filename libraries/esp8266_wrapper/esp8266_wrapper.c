...
int i2c_slave_write(uint8_t bus, uint8_t addr, uint8_t *reg,
                    uint32_t regLen, uint8_t *data, uint32_t len) {
...
int i2c_slave_read(uint8_t bus, uint8_t addr, uint8_t *reg, uint32_t regLen, uint8_t *data,
                   uint32_t len) {
  if (len == 0)
    return true;
  ...
  esp_err_t err = i2c_master_cmd_begin(bus, cmd, 5000 / portTICK_RATE_MS);
  i2c_cmd_link_delete(cmd);
